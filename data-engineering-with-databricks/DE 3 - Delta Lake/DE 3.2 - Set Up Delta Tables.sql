...
select '${DA.paths.datasets}/ecommerce/raw/sales-historical';
--dbfs:/mnt/dbacademy-datasets/data-engineer-learning-path/v02/ecommerce/raw/sales-historical

create or replace table pna_sales as 
select * from parquet.`${DA.paths.datasets}/ecommerce/raw/sales-historical`;

describe extended pna_sales;
...
create table pna_sales_unparsed as 
select * from csv.`${da.paths.datasets}/ecommerce/raw/sales-csv`;

SELECT * FROM pna_sales_unparsed;
...
CREATE OR REPLACE TEMP VIEW pna_sales_tmp_vw
  ...
  create table pna_sales_delta as 
  select * from pna_sales_tmp_vw;

  select * from pna_sales_delta;
...
create or replace table pna_purchase_dates (
  id STRING, 
  transaction_timestamp STRING, 
  price STRING,
  pna_txt string generated always as ("sth") comment "generated test",
  pna_date DATE generated always as (
    cast(cast(transaction_timestamp/1e6 as TIMESTAMP) as date) ) comment "generated transformed date"
);


--select * from pna_purchase_dates;
--describe extended pna_purchase_dates;
describe  pna_purchase_dates;
...
--pna
 ...
 (9999999, 1592193956703494, 42.0, "2020-06-15")

-- COMMAND ----------

 INSERT INTO purchase_dates VALUES
 (1, 600000000, 42.0, "2020-06-18")
 --generates error correctly
...
alter table pna_sales_delta add constraint pna_temp_check check(order_id >= 0)
