...
create schema if not exists pna_delme;
drop schema pna_delme;
...
--PNA
--select * from pna_managed_table;
--describe table extended pna_managed_table;
describe detail pna_managed_table;
...
-- MAGIC #PNA
-- MAGIC
-- MAGIC table_name = "pna_managed_table"
-- MAGIC table_location = spark.sql("DESCRIBE DETAIL pna_managed_table").first().location
-- MAGIC
-- MAGIC #display(table_location)
-- MAGIC
-- MAGIC files = dbutils.fs.ls(table_location)
-- MAGIC display(files)
...
--select "${DA.schema_name} " --pavol_nagl_yyuo_da_delp
--select "${da.paths.datasets}" --dbfs:/mnt/dbacademy-datasets/data-engineer-learning-path/v02
--'${da.paths.datasets}/flights/departuredelays.csv' >> --dbfs:/mnt/dbacademy-datasets/data-engineer-learning-path/v02/flights/departuredelays.csv

USE pavol_nagl_yyuo_da_delp_default_location;

create or replace temporary view pna_temp_delays using CSV options (
  path = '${da.paths.datasets}/flights/departuredelays.csv',
  header = "true",
  mode = "FAILFAST"
);

create or replace table pna_external_table location '${da.paths.working_dir}/external_table' as 
  select * from pna_temp_delays;

  select * from pna_external_table;
