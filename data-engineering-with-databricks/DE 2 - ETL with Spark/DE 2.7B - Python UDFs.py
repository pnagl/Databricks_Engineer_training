...
def pna_first_letter_function(txt):
      return txt[0]
    
pna_first_letter_function("sla")
...
pna_first_letter_udf = udf(pna_first_letter_function)
...
sales_df.select(pna_first_letter_udf(col("email")),"email").display()
...
#PNA
@udf("string")
def pna_first_letter_udf(txt:str) -> str:
  return txt[0]
...
sales_df = spark.table("sales")
sales_df.select(pna_first_letter_udf(col("email"))).display()
