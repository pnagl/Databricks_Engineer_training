...
-- MAGIC
-- MAGIC pna_events_raw = (spark.table("events_raw"))
-- MAGIC #pna_events_raw_desc = pna_events_raw.schema()
-- MAGIC
-- MAGIC pna_events_stringDF = (
...
-- MAGIC
-- MAGIC
-- MAGIC pna_events_stringDF.limit(3).display()
-- MAGIC pna_events_stringDF.printSchema()
-- MAGIC
...
"device":"Linux"
,"ecommerce":
  {"purchase_revenue_in_usd":1047.6,"total_item_quantity":2,"unique_items":2}
,"event_name":"finalize"
...
,"items":
  ...
  ,{"coupon":"NEWBED10","item_id":"P_DOWN_S","item_name":"Standard Down Pillow","item_revenue_in_usd":107.10000000000001,"price_in_usd":119.0,"quantity":1}
  ]
...
--PNA
SELECT value:event_name, value:device, t.* FROM events_strings t WHERE value:event_name = "finalize" ORDER BY key LIMIT 1
...
-- MAGIC #PNA
-- MAGIC
-- MAGIC display(pna_events_stringDF.where("value:event_name = 'finalize'")
-- MAGIC                            .orderBy("key")
-- MAGIC                            .limit(1)
-- MAGIC                            )
-- MAGIC
...
-- MAGIC """
-- MAGIC
-- MAGIC pna_json_schema = schema_of_json(pna_json_string)
-- MAGIC
-- MAGIC pna_parsed_eventsDF = (
-- MAGIC   pna_events_stringDF
-- MAGIC       .select(from_json("value",schema_of_json(pna_json_string)).alias("PNA_json"))
-- MAGIC       .select("PNA_json.*")
-- MAGIC )
-- MAGIC
-- MAGIC pna_parsed_eventsDF2 = (
-- MAGIC   pna_events_stringDF
-- MAGIC       .select(from_json("value",pna_json_schema ).alias("PNA_json"))
-- MAGIC       .select("PNA_json.*")
-- MAGIC )
-- MAGIC
-- MAGIC
-- MAGIC
-- MAGIC
-- MAGIC #display(pna_json_string)
-- MAGIC #display(pna_parsed_eventsDF)
-- MAGIC
-- MAGIC #display(pna_json_schema)
-- MAGIC display(pna_parsed_eventsDF2)
...
--select * from parsed_events;
select * from parsed_events t where size(t.items) > 2;

select explode(items), * from parsed_events where size(items) > 2;
...
-- MAGIC
-- MAGIC pna_exploded_items = (
-- MAGIC   pna_parsed_eventsDF.withColumn("item",explode("items"))
-- MAGIC )
-- MAGIC
-- MAGIC (
-- MAGIC pna_exploded_items.where(size("items") >2)
-- MAGIC                   #.limit(2)
-- MAGIC                   .display()
-- MAGIC )
-- MAGIC
-- MAGIC pna_exploded_items.describe()
-- MAGIC
...
FROM exploded_events
 where user_id = 'UA000000107380131'
GROUP BY user_id
...
-- MAGIC #PNA
-- MAGIC
-- MAGIC from pyspark.sql.functions import array_distinct, flatten, collect_set
-- MAGIC
-- MAGIC pna_collected_items = (
-- MAGIC     pna_exploded_items.groupby("user_id")
...
-- MAGIC )
-- MAGIC
-- MAGIC #pna_exploded_items.limit(3).display()
-- MAGIC pna_collected_items.display()
-- MAGIC
...
-- MAGIC
-- MAGIC pna_exploded_sales = (
-- MAGIC   spark.table("sales")
-- MAGIC        .withColumn("item",explode("items"))
-- MAGIC )
-- MAGIC
-- MAGIC pna_item_lookup = (
-- MAGIC   spark.table("item_lookup")
-- MAGIC )
-- MAGIC
-- MAGIC pna_item_purchasesDF = (
-- MAGIC   pna_exploded_sales.join(pna_item_lookup,pna_exploded_sales.item.item_id == pna_item_lookup.item_id)
-- MAGIC )
-- MAGIC
-- MAGIC #display(pna_exploded_sales.limit(3))
-- MAGIC #display(pna_item_lookup.limit(3))
-- MAGIC display(pna_item_purchasesDF)
