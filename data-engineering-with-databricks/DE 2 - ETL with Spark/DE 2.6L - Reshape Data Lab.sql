...
--PNA Solution!
CREATE OR REPLACE TEMP VIEW events_pivot_nongroup as 
select * from events
...
pivot(count(*) 
  for event_name 
  in( "cart", "pillows", "login", "main", "careers", "guest", "faq", "down", "warranty", "finalize", "register", "shipping_info", "checkout", "mattresses", "add_item", "press", "email_coupon", "cc_info", "foam", "reviews", "original", "delivery", "premium") 
  )
...
-- MAGIC                 , "traffic_source"
-- MAGIC                 , "user_first_touch_timestamp"
-- MAGIC                 , "user_id")
-- MAGIC        .pivot("event_name")
-- MAGIC        .agg(count("device"))
-- MAGIC        .createOrReplaceTempView("events_pivot")
-- MAGIC )
-- MAGIC
-- MAGIC #display(pna_DF).limit(3)
-- MAGIC #pna_DF.describe()
-- MAGIC spark.table("events_pivot").select("*").display()
...
-- MAGIC        .agg(count("device"))
-- MAGIC        .withColumnRenamed("user_id","user")
-- MAGIC        .select("user","cart", "pillows", "login", "main", "careers", "guest", "faq", "down", "warranty", "finalize", 
...
-- MAGIC   )
-- MAGIC
-- MAGIC #spark.table("events_pivot").display()
