...
-- MAGIC %python
-- MAGIC display(DA.paths.sales_csv)
...
-- MAGIC #PNA
-- MAGIC
-- MAGIC display(DA.paths.ecommerce_db)
-- MAGIC #print("\n")
-- MAGIC ## neviem ako zobrazit co je v tom subore # print(/dbfs/mnt/dbacademy-users/pavol.nagl@gmail.com/data-engineer-learning-path/ecommerce.db\)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC
-- MAGIC #--SELECT * FROM text.`${DA.paths.ecommerce_db}`
-- MAGIC #--select 'asd'
-- MAGIC print (f'${DA.paths.ecommerce_db}')
-- MAGIC print("\n")
-- MAGIC display(f'${DA.paths.ecommerce_db}')
...
-- MAGIC #print(location)
-- MAGIC
-- MAGIC v_table = spark.sql("DESCRIBE EXTENDED users_jdbc")
-- MAGIC
-- MAGIC #display(v_table)
-- MAGIC v_location_txt = v_table.filter( F.col("col_name") == "Location").first()["data_type"]
-- MAGIC display(v_location_txt)
-- MAGIC
-- MAGIC #files = dbutils.fs.ls(location)
-- MAGIC #print(f"Found {len(files)} files")
