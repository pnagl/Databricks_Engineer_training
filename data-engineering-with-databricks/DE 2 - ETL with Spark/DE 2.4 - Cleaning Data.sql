...
--select * from users_dirty t where t.email is null
select * from users_dirty t where t.user_id is null
...
-- MAGIC
-- MAGIC #NOK#pnaDF = spark.read.sql("select * from users_dirty")
-- MAGIC pnaDF = spark.read.table("users_dirty")
-- MAGIC #display(pnaDF)
-- MAGIC
-- MAGIC pnaDF.selectExpr("count_if(email IS NULL)")
-- MAGIC display(pnaDF.selectExpr("count_if(email IS NULL)"))
-- MAGIC pnaDF.where(col("email").isNull()).count()
-- MAGIC display(pnaDF.where(col("email").isNull()).count())
...
-- MAGIC %python 
-- MAGIC #PNA
-- MAGIC # Accessing the PySpark documentation
-- MAGIC # Follow this link: https://spark.apache.org/docs/latest/api/python/
-- MAGIC # It leads to the PySpark 3.2.0 documentation. 
-- MAGIC
-- MAGIC from pyspark.sql import *
-- MAGIC
-- MAGIC pnaDF_Dedup = (
-- MAGIC   pnaDF#.select("user_id")
-- MAGIC        .where(col("user_id").isNotNull())
-- MAGIC        .groupBy(col("user_id"), col("user_first_touch_timestamp"))
-- MAGIC        .agg(count(col("user_first_touch_timestamp")).alias("cnt")
-- MAGIC             ,max("email").alias("email")
-- MAGIC             ,max("updated").alias("updated")  
-- MAGIC          )
-- MAGIC )
-- MAGIC
-- MAGIC #pnaDF_Dedup.where(col("cnt")>1).display()
-- MAGIC
-- MAGIC
-- MAGIC #display(pnaDF_Dedup.count())
-- MAGIC #display(pnaDF_Dedup)
-- MAGIC
-- MAGIC #display(pnaDF_Dedup.where(col("cnt")>2)
-- MAGIC display(pnaDF.where(col("user_id") == "UA000000107383284"))
-- MAGIC
...
-- MAGIC #PNA
-- MAGIC
-- MAGIC pnaDF2 = spark.sql("select * from users_dirty")
-- MAGIC
-- MAGIC pnaDF2_Dedup = (
-- MAGIC   pnaDF2.where(col("user_id").isNotNull())
-- MAGIC         .groupBy(col("user_id"),col("user_first_touch_timestamp"))
-- MAGIC         .agg(max(col("email")).alias("email"),max("updated").alias("updated"))
-- MAGIC )
-- MAGIC
-- MAGIC pnaDF2_Dedup.display()
-- MAGIC
...
-- MAGIC
-- MAGIC #pnaDF.display()
-- MAGIC (pnaDF.dropDuplicates(["user_id", "user_first_touch_timestamp"])
...
-- MAGIC #TODO
-- MAGIC (
-- MAGIC   pnaDF2_Dedup.groupBy("user_id")
-- MAGIC               .agg(count('*').alias("row_count"))
-- MAGIC               #.filter(col("row_count") < 2)
-- MAGIC               .select( (max("row_count")<=1).alias("Max_1_rowcount?") )
-- MAGIC               
-- MAGIC               .display()
-- MAGIC               #.show()
-- MAGIC  
...
-- MAGIC
-- MAGIC '''
-- MAGIC SELECT max(row_count) <= 1 no_duplicate_ids FROM (
-- MAGIC   SELECT user_id, count(*) AS row_count
...
-- MAGIC
-- MAGIC (
-- MAGIC   pnaDF2_Dedup.where(col("email").isNotNull())
-- MAGIC               .groupBy("email")
...
-- MAGIC (
-- MAGIC   pnaDF2_Dedup
-- MAGIC               .withColumn("PNA_First_touch", (col("user_first_touch_timestamp")/1e6 ).cast("timestamp"))
-- MAGIC               .withColumn("PNA_First_touch_date", date_format("PNA_First_touch","MMM d yyyy") )
-- MAGIC               .withColumn("PNA_First_touch_time", date_format("PNA_First_touch","kk:mm:ss") )
-- MAGIC               .withColumn("PNA_Domain", regexp_extract('email', "(?<=@).+",0)  )
-- MAGIC               .select("PNA_First_touch","PNA_First_touch_date","PNA_First_touch_time","user_first_touch_timestamp","PNA_Domain","email")
-- MAGIC               .limit(3)
-- MAGIC               .display()
-- MAGIC
-- MAGIC )
